                ...
                if isinstance(token, Parenthesis):
                    ...
                    for col_def in col_defs:
                        ...
                        if isinstance(col_def, IdentifierList):
                            print(f"col IdentifierList : {col_def} End")
                            ...
                            print(f"col Identifier : {col_def} End")
...
def generate_alter_statements(table_name, add_columns, remove_columns, modify_columns):
    ...
    for col_name, col_def in modify_columns.items():
        alter_statements.append(f"ALTER TABLE {table_name} ALTER COLUMN {col_name} TYPE {col_def.split()[1]};")
...
def generate_reverse_alter_statements(table_name, add_columns, remove_columns, modify_columns, old_columns):
    reverse_statements = []
    ...
    for col_name, col_def in modify_columns.items():
        old_col_def = old_columns[col_name]
        reverse_statements.append(f"ALTER TABLE {table_name} ALTER COLUMN {col_name} TYPE {old_col_def.split()[1]};")
...
def main(old_ddl, new_ddl):
    ...
    reverse_alter_statements = generate_reverse_alter_statements(table_name, add_columns, remove_columns, modify_columns, old_columns)
