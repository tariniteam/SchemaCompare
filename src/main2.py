...
def generate_reverse_alter_statements(table_name, add_columns, modify_columns):
    reverse_statements = []

    for col_name in add_columns.keys():
        reverse_statements.append(f"ALTER TABLE {table_name} DROP COLUMN {col_name};")

    for col_name in modify_columns.keys():
        reverse_statements.append(f"ALTER TABLE {table_name} ALTER COLUMN {col_name} DROP DEFAULT;")
...
def main(old_ddl, new_ddl, table_name):
    ...
    reverse_alter_statements = generate_reverse_alter_statements(table_name, add_columns, modify_columns)

    print("ALTER statements to update the table:")
    for stmt in alter_statements:
        print(stmt)

    print("\nALTER statements to reverse the changes:")
